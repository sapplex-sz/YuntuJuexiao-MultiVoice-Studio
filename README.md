...
| 持久化音色库 | 已用音色可命名保存，重启服务后仍可从列表直接选择，无需再次上传和转写 |
| 30 个中文内置音色 | 从 Apache 2.0 的 AISHELL-3 精选匿名说话人参考片段，覆盖男女、年龄段和南北方口音 |
| 先听再选 | 独立音色库支持搜索、原声试听，并一键应用到指定角色 |
| 成品台词核对 | 按台词长度限制推理，再用本地 Whisper 时间戳核对台词、裁去无关尾音；异常成品不发布 |
...
3. 说话人数要与脚本中出现的角色一致。
4. 在右侧选择“自动”时长（推荐），点击“生成语音”，完成后试听或下载 WAV。

创作台把台词、角色选声与作品输出分区展示。生成上限随台词长度变化，15/30/60 秒选项是上限而非精确时长。生成完成后，本地 Whisper 将录音与输入台词进行前缀匹配，并按词级时间戳裁掉多余尾音。核对增加少量耗时；若识别失败、台词遗漏或无法确认结束点，页面会提示重试，不返回未经核对的音频。识别可能受人名、口音等影响，因此仍建议最终人工试听；该检查不保证逐字零误差。

### 使用参考音色

1. 在“角色与声音”中直接选择音色并试听，或展开该角色的“使用自己的录音 / 编辑参考原文”。
...
1. 上传参考音频，等待自动识别原文并完成核对。
2. 勾选“我拥有此录音的声音使用授权”。
3. 输入便于辨认的音色名称，点击“保存到我的音色”。
4. 下次直接在角色的音色列表中选择，音频和原文会自动填入。

“音色库 · 先听再选”页签可以搜索和播放全部内置/个人参考片段。选择目标角色，再点击卡片上的“使用这个音色”，会自动返回创作台并应用。原声试听不运行生成模型，和最终合成效果可能有差异。

音色文件和索引默认保存在 `voice_library_data/`，不会提交到 Git。可通过环境变量 `YUNTU_VOICE_LIBRARY_DIR` 把它放到独立数据盘或备份目录。

### 安装 30 个开源中文音色

平台使用 AISHELL-3 测试集中的匿名参考片段作为内置音色。安装器只下载所需短音频，不会下载完整的 19GB 数据集：

```powershell
.\.venv\Scripts\python.exe scripts\install_builtin_voices.py --count 30
```

安装后点击页面中的“刷新音色列表”，或重启服务。每个内置音色都记录 AISHELL-3 说话人编号、年龄段、性别、口音、来源链接和 Apache 2.0 许可证。详细归属见 [VOICE_LIBRARY_NOTICE.md](./VOICE_LIBRARY_NOTICE.md)。
...
├── voice_library.py            # 持久化音色、索引、授权与路径安全
...
│   ├── install_builtin_voices.py # 安装 30 个 AISHELL-3 中文音色
...
内置音色来自为多说话人语音合成研究发布的匿名数据，但数据许可证不等同于对所有场景下人格权、声音权益或当地法律义务的豁免。对外发布或商业使用前，请根据实际地区和用途完成合规评估。
