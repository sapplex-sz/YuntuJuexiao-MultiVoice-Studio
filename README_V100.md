...
网页默认使用简体中文，分为「创作台」和「音色库 · 先听再选」。创作台包含台词、角色与声音、作品输出三个区域；录音编辑和高级参数默认折叠。点击「单人朗读」「双人对话」或「播客开场」可加载示例，同时清空已有参考音色。
...
点击「生成语音」，完成后在右侧试听或下载。角色可直接选择内置音色并试听，或展开「使用自己的录音 / 编辑参考原文」上传录音，服务器自动转写。默认按台词估算生成上限；15/30/60 秒选项不会拉长音频。成品通过本地 Whisper 核对后按台词结束点清理尾音；核对失败会拦截输出并提示重试。此功能需要已安装的 `.venv-asr` 和 Whisper 模型，可能因识别误差误拦截，最终仍需人工试听。
...
上传音频并核对原文后，可命名保存；下次直接从角色音色列表使用。音色库中每个卡片可独立试听，并一键应用到指定角色。数据默认保存在 `C:\AI\MOSS-TTSD\voice_library_data`，服务重启不会丢失。建议定期备份该目录，也可在启动服务前通过 `YUNTU_VOICE_LIBRARY_DIR` 指定独立数据盘。
